...
## File Conventions (post-split correction, 2026-05-22)

The original File Conventions section above (lines 87-96) referenced
`/projects/<project-name>/` and `/portfolio/`. Both have moved out:

- **Subneτ Magazine** (was `/projects/subnet-magazine-v2/`) lives
  at https://github.com/RondoAI/subnet-magazine — see the
  "Subneτ Magazine MOVED" section at the bottom of this file.
- **Paper portfolio** (was `/portfolio/`) is subnet/market data
  per Rondo 2026-05-22 ("that is subnet data, it goes in subnet
  folder") and moved with the magazine.

This curriculum repo is now scoped to ONE thing only: the Python
self-education record. The only AI-market artifact that stays
here is `/briefings/` (each daily briefing ends with a learning
task tied to the day's news — they are a learner artifact, not
magazine product). Per Rondo 2026-05-22: "the only thing about
AI markets that should stay is the daily briefing."

Current top-level layout:

  curriculum/      Python phase folders (phase-1-foundations, ...)
  briefings/       YYYY-MM-DD.md daily AI-market briefings, each
                   ending with one learning task
  notes/           Rondo's dated notes in his own words
  prompts/         Session-mode prompts (boot, journal, teaching,
                   master_mission, etc.)
  writings/        Longer-form writing
  *.md             Learning record at the root (JOURNAL, PROGRESS,
                   SESSION_LOG, LEARNER_PROFILE, MILESTONES,
                   CHRONICLE, CONCEPTS)
  index.html       The Field Manual landing page
  README.md        Public-facing entry point — Python curriculum

## Subneτ Magazine MOVED to its own repo (2026-05-22) ⭐ BINDING

Rondo's instruction, 2026-05-22: "Subnet Magazine work should be
separate from Python study on GitHub. These two projects are
totally different and should have been in two separate folders."
Plus: "Python education should be protected and not mixed with
other things. Also it should be the main thing people see on
GitHub."

The Subneτ Magazine project has been extracted to a dedicated
repo: **https://github.com/RondoAI/subnet-magazine**

### What happened

1. New repo `RondoAI/subnet-magazine` created via `gh repo create`
   (public, scoped to magazine work only).
2. `git subtree split --prefix=projects/subnet-magazine-v2 -b
   magazine-only` extracted 594 magazine-touching commits with
   full history preserved.
3. Pushed `magazine-only` to the new repo as `main`. Files that
   lived at `projects/subnet-magazine-v2/cockpit.html` now live
   at `cockpit.html` (top-level) in the new repo.
4. The `projects/subnet-magazine-v2/` directory in this curriculum
   repo has been REPLACED with a single `MIGRATED.md` file
   pointing readers at the new repo.
5. All Subneτ-Magazine-scoped CLAUDE.md rules (NodeSphere E8 lock,
   "don't mess up anything" pre-flight, const founder feedback,
   128-subnet curation bindings, monetization plan, coordination
   logs, etc.) MOVED with the magazine to the new repo's
   CLAUDE.md. None remain in this file.

### URLs

  PAST (commit-pinned URLs from before the split still work for
  any past SHA — git history preserves the files at those SHAs):
    raw.githack.com/RondoAI/rondo-AI-curriculum/<sha>/projects/subnet-magazine-v2/cockpit.html

  PRESENT (latest-branch URL serves the MIGRATED.md pointer):
    raw.githack.com/RondoAI/rondo-AI-curriculum/main/projects/subnet-magazine-v2/MIGRATED.md

  FUTURE (where all new magazine work lives):
    raw.githack.com/RondoAI/subnet-magazine/main/cockpit.html
    raw.githack.com/RondoAI/subnet-magazine/<sha>/cockpit.html

### Binding rules going forward

  1. NO new magazine commits in this repo. All future cockpit /
     curation / Oracle work pushes to RondoAI/subnet-magazine.
     Both `mac-session` and `sandbox-session` switch remotes for
     magazine work.
  2. The `subnet-mag-v2-upgrades` and `subnet-mag-v2` branches in
     THIS repo are frozen as of 2026-05-22. Continue magazine
     work on the new repo's `main`.
  3. This file (curriculum CLAUDE.md) keeps ONLY the Python
     curriculum + learning rules. Adding any magazine-scoped
     rule here would re-merge what Rondo explicitly asked to
     split.
  4. Daily briefings (`/briefings/`) stay in curriculum. The
     magazine can pull briefings via a published feed when
     wired in a future pass.

### For sibling on next boot

When you `git pull` this repo and notice it's slim, that's
correct — magazine work migrated. Switch your remote for any
magazine work:

  git clone git@github.com:RondoAI/subnet-magazine.git

The full coordination history (sandbox-session ↔ mac-session
threads) lives in the new repo's CLAUDE.md.

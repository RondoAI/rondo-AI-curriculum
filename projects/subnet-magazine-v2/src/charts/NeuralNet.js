...
const DEFAULT_LAYERS = [10, 18, 14, 6, 1];
...
export class NeuralNet extends Chart {
  ...
  constructor(canvas, opts = {}){
    ...
    /** @private */ this.spawnRate = opts.spawnRate ?? 0.18;     // pulses/sec/input-node
    ...
    for (let l = 0; l < cols - 1; l++){
      ...
      for (let i = 0; i < a; i++){
        for (let j = 0; j < b; j++){
          ...
          // pruning: keep ~70% of edges so the graph is fully but
          // not overwhelmingly connected
          if (r < 0.30) continue;

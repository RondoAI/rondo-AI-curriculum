...
export const SUBNET_BIOS = Object.freeze([
  ...
  {
    netuid: 8,
    oneline: 'Decentralized proprietary trading network with live PnL scoring.',
    ...
    bio: 'PTN — now publicly trading under the Vanta brand — is Taoshi\'s decentralized prop-trading harness. Miners submit live trading agents (positions on BTC, ETH, FX, equity indices, commodities); validators track every fill on a 24/7 paper-execution layer and rank by Sharpe-adjusted PnL, drawdown, and turnover. Top deciles earn α; bottom deciles get culled. The thesis is that the protocol can outbid centralized prop firms (FTMO, Topstep) by paying out emissions before any client capital is deployed, and then monetize the top-ranked agents through funded accounts. Q1 2026 went live February 17 with three consumer surfaces — VantaTrading for retail, GlitchFinancial for automated execution (public launch Q1), and 0x_Markets DEX for the underlying spot fills — plus a stated March 31 target for Hyperscaled, a Hyperliquid-integrated prop-firm rail. Emission share ~3.5%, momentum positive over Q2. Find it at taoshi.io and vanta.trading; repo at taoshidev/proprietary-trading-network.'
  },

  // RANK 8
  {
    netuid: 5,
    oneline: 'Decentralized text embeddings and web search retrieval.',
    keyMetric: '~3.1% emission share',
    recentNews: 'Yaps integration rolling out in Q2 2026, plugging SN5 embeddings into Kaito\'s mainstream crypto-mindshare product.',
  ...
  {
    netuid: 19,
    oneline: 'Ultra-low-latency open-source LLM and image inference.',
    ...
    bio: 'Nineteen is the third leg of the Rayon Trio: a latency-obsessed inference subnet built for use cases where milliseconds matter — trading agents, live translation, real-time gameplay, voice agents. Miners are scored almost entirely on tok/s and TTFT under load; quality is a floor, not the ranking axis. The subnet holds the public world record for fastest open-source LLM serving (set in late 2024, defended through 2026), beating centralized incumbents including Perplexity\'s sonar tier on identical models. Operationally, Rayon routes Chutes\' time-critical workloads here, which gives Nineteen a steady real-traffic profile most subnets lack. Emission share ~2.7%, smaller in τ terms than Chutes or Gradients but punching above its weight on product proof. Competes against Groq, Cerebras, and SambaNova on the high-end; against Together AI and Fireworks at parity hardware. Repo at rayonlabs/nineteen.'
  },
  ...
  {
    netuid: 120,
    oneline: 'Multi-subnet RL coordination and model-improvement layer.',
    ...
    bio: 'Affine is a coordination layer rather than a single-domain producer. It runs winner-takes-all reinforcement-learning competitions across well-defined task families — code generation, program abduction, math reasoning — where any participant can pull the current best model, propose an incremental improvement, and submit it back. Validators run the improved model against the task suite; if it materially beats the incumbent, the contributor earns the full reward for that round. The architecture deliberately leans on Chutes for hosting, on Gradients for training, on Lium for compute, which makes Affine a piece of meta-infrastructure that compounds on whatever the rest of the network produces. As of March 25, 2026, α-market cap was reported around $71.8M. Emission share has rotated through the top three more than once in 2026, occasionally displacing Chutes at the top. Repo at AffineFoundation/affine-cortex.'
  },
  ...
  // RANK 11
  {
    ...
    bio: 'Ridges is the SWE-agent subnet: miners ship autonomous coding agents that pull tickets from a private task harness (SWE-bench-style scaffolding plus internal benchmark sets), edit a sandboxed repo, and submit diffs. Validators run the resulting code, score on pass-rate, diff size, and execution cost, and rank. The product target is explicit — match Claude Code or Cursor on simple-to-medium engineering work, at $2-5/day instead of $50+/seat — and the incentive flywheel is designed to compound iteratively-improving agents on top of an open codebase. Q1 2026 milestones: full rewrite of miner tooling, public agent-output dashboard, and a new incentive mechanism announcement teed up for Q2. Mindshare ranking has been the highest of any Bittensor subnet through 2026 according to public mindshare trackers, though emission share is more modest at single-digit percent. Find it at ridges.ai; repo at ridgesai/ridges.'
  },
  ...
  // RANK 12
  {
    ...
    bio: 'Apex is the original miner battleground — the OG SN1 — and after several mechanism overhauls it now runs as Macrocosmos\' open-domain conversational layer. Miners serve open-weight LLMs (Llama, Mistral, DeepSeek) augmented with agentic scaffolding: tool use, code execution, live web search through SN5. Validators send synthetic and human-labeled prompts, then score on factuality, coherence, and reasoning depth against a rotating rubric. Macrocosmos exposes the subnet through the Apex SDK and a Mission Command web console, with rate limits of 100 req/h per public key and 1,000/h for validators. Late-2025 milestone: Battleships v1, the first fully peer-to-peer in-subnet competition, ran on Apex with winners paid in SN1 α — a template Macrocosmos has been extending into other game-shaped competitions through 2026. Emission share sits in the mid-single digits. Find it at macrocosmos.ai; repo at macrocosm-os/apex.'
  },

  // RANK 13
  {
    netuid: 44,
    oneline: 'Sports computer vision and match-outcome prediction.',
  ...
  {
    netuid: 13,
    oneline: 'Decentralized social-data scraping layer for the AI stack.',
    keyMetric: '55B+ scraped rows, ~350M new rows/day',
    recentNews: 'Score (SN44) signed on as a downstream customer in Q1 2026; queryable Endpoints API in active development.',
    bio: 'Data Universe is the data-supply layer of the Macrocosmos cluster — the gravity well that feeds everything else. Miners scrape Reddit, X, and (increasingly) YouTube transcripts and TikTok metadata; validators audit row provenance, dedupe, and rank by freshness and demand-side relevance. The subnet now holds upwards of 55 billion scraped posts and comments and ingests roughly 350 million new rows per day. The strategic shift in 2026 has been from raw scraping to a queryable Endpoints API that downstream subnets and external customers can hit directly — SN44 Score signed on as a Q1 anchor. Macrocosmos positions this as the data-pipeline twin to Apex (SN1) and IOTA (SN9): the same team controls collection, training, and serving. Emission share sits in the mid-single digits, but the integration moat across Macrocosmos subnets makes it strategically punching above raw rank. Repo at macrocosm-os/data-universe.'
  },

  // RANK 15 — Post-Covenant rebuild, community-operated.
  {
    netuid: 39,
    oneline: 'Agent-native sandboxed compute for AI workloads.',
    keyMetric: 'ATH $8.02 on April 6, 2026; subsequently drawn down 70%+',
    recentNews: 'Deprecated by Covenant AI on April 9, 2026; community miners restarted the subnet from open-source code within days.',
    bio: 'Basilica was relaunched in January 2026 by Covenant AI founder Sam Dare as an agent-native compute layer — sandboxed, ephemeral, disposable runtimes that AI agents could spin up in seconds rather than the minutes-to-hours overhead of generic GPU rentals. The product distinction from Chutes is environmental: Basilica targets agentic workloads that need to install packages, run untrusted code, write files, and self-terminate, where Chutes targets stable inference endpoints. The subnet hit an α-price ATH of $8.02 on April 6, 2026, then was deprecated four days later when Covenant AI exited the network on April 9 amid an acrimonious governance dispute. Community miners reconstituted the subnet from the open-source repo within days; it now runs without central operator coordination. Currently rebuilding emission share from the post-exit drawdown of roughly 70%+. Repo at tplr-ai/basilica.'
  },

  // RANK 16 — Post-Covenant rebuild, community-operated.
  {
    netuid: 81,
    oneline: 'Cooperative RL post-training of a shared LLM.',
  ...
  {
    netuid: 68,
    oneline: 'Decentralized drug-discovery screening of molecular compounds.',
    keyMetric: '11M+ molecules screened against ~7,000 protein targets',
    recentNews: 'Crossed 11 million screened molecules on May 10, 2026, a milestone for decentralized pharma R&D.',
  ...
  {
    netuid: 75,
    oneline: 'Decentralized S3-equivalent storage for the AI stack.',
    keyMetric: '~$41.3M α-market cap (late March 2026)',
    recentNews: 'Posted +115% 30-day α-price gain through the late-March 2026 ecosystem rally; positioned as Bittensor\'s sovereign-cloud storage layer.',
    bio: 'Hippius is Bittensor\'s decentralized storage layer — the equivalent of S3 for a network that, until early 2025, had no real way to persist data across workloads. Miners commit storage capacity (NVMe and HDD tiers), validators run periodic challenge-response audits (proof-of-storage interactive checks plus random read sampling) and rank by uptime, retrieval latency, and verified capacity. All usage and payment is recorded on-chain, which is the regulated-buyer pitch. The subnet was introduced in March 2025 as the "missing piece" for dApps that needed real persistence, and through 2026 it has become the default storage backend for several other subnets including agent runtimes. α-market cap reached approximately $41.3M in late March 2026 with a 115% 30-day move; the project benchmarks against Filecoin and Arweave on decentralized storage, and against Cloudflare R2 on price. Find it at hippius.com.'
  },
  ...
  // RANK 19
  {
    ...
    bio: 'Sturdy is the cleanest revenue-bearing DeFi subnet: miners submit yield-optimization strategies that allocate USDC and ETH across whitelisted lending pools (Aave, Compound, Morpho), validators score against live PnL with a continuous backtest harness, and the top-ranked strategy is the one Sturdy\'s aggregator actually executes on-chain. The product feeds a real Sturdy vault on Ethereum mainnet, which by 2026 had over $125M in allocated deposits — the rare case where dTAO emission and real client capital flow through the same scoring pipeline. Q1 2026 milestone: a partnership with Morpho on a co-branded AI-optimized lending vault, plus the Gauntlet integration that adds risk-adjusted scoring. Competes against Yearn, Sommelier, and Gauntlet itself on the strategy side. Emission share sits in the low single digits but external TVL is the real metric. Find it at sturdy.finance; repo at Sturdy-Subnet/sturdy-subnet.'
  },
  ...
  {
    netuid: 34,
    oneline: 'Self-evolving deepfake and synthetic-media detection.',
    ...
    bio: 'BitMind runs the generative-adversarial economy: detector miners train classifiers that flag AI-synthesized images, video, and audio; generator miners produce new synthetic samples designed to slip past current detectors; validators score the resulting battle and pay both sides asymmetrically. The architecture, which BitMind brands the Generative Adversarial Subnet (GAS), is structurally a moving-target defense that should harden against each new image and video model that ships externally. Reported accuracy sits around 95% on standardized benchmarks, with sub-second response latency and SOC 2 compliance — both of which are unusual for a Bittensor subnet and aimed at insurance, content moderation, and regulated-media buyers. Q1 2026 saw expansion into a hosted detection app and an ElizaOS plugin for agent-stack integration. Competes against Reality Defender, Hive Moderation, and Microsoft\'s content credentials stack. Find it at bitmind.ai; repo at BitMind-AI/bitmind-subnet.'
  },
  ...
  {
    netuid: 25,
    oneline: 'Decentralized protein folding and molecular simulation.',
  ...
  {
    netuid: 18,
    oneline: 'Decentralized climate and weather forecasting on ERA5.',
    keyMetric: 'ERA5 reanalysis-graded forecasts at hourly resolution',
    recentNews: 'Now expanding beyond 2-meter surface temperature into multi-variable forecasting on the full Copernicus ERA5 stack.',
  ...
  {
    netuid: 6,
    oneline: 'Continuous finetuning competition on rotating benchmarks.',
    keyMetric: 'First continuous fine-tuning benchmark on Bittensor',
    recentNews: 'Operating as Numinous in 2026, with a continuously updated synthetic-data stream as the held-out evaluation set.',
  ...
  {
    netuid: 2,
    oneline: 'Zero-knowledge verifiable inference proofs at scale.',

...
/* No JS helper needed — recoloring happens in CSS via a
   filter recipe applied to the logo <img>. The CSS-mask
   approach was cross-origin restricted (most chain
   logoUrls live on different origins than the cockpit). */
        ...
        <div class="cock-chart__title">
          ...
          <h1 class="cock-chart__h">
            <span class="cock-chart__logo" aria-hidden="true">
              ${(() => {
                /* Curated marks (Hippius PNG, Targon SVG) ship in
                   /assets and were illustrated in our palette
                   already. Every other subnet shows its OWN brand
                   logo (chain logoUrl) inside the Hippius-style
                   frame (midnight bg + red border applied by the
                   wrapper) — the frame provides the cockpit\'s
                   visual cohesion across 128 subnets, each team
                   keeps its brand identity. */
                const curated = SUBNET_LOGOS[(s.name || '').toLowerCase()];
                const src = curated || s.logo || FALLBACK_LOGO;
                return `<img class="cock-chart__logo-img" src="${src}" alt="" loading="lazy" onerror="this.src='${FALLBACK_LOGO}'">`;
              })()}
            </span>
        ...
        <tr class="cock-fold-mkt__row" data-roster-row="${x.netuid}" tabindex="0" role="button" aria-label="Open SN${x.netuid} ${x.name} chart">
          ...
          <td class="cock-fold-mkt__name">
            <span class="cock-fold-mkt__logo" aria-hidden="true">
              ${(() => {
                const curated = SUBNET_LOGOS[(x.name || '').toLowerCase()];
                const src = curated || x.logo || FALLBACK_LOGO;
                return `<img class="cock-fold-mkt__logo-img" src="${src}" alt="" loading="lazy" onerror="this.src='${FALLBACK_LOGO}'">`;
              })()}
            </span>
            ${x.name}
          </td>
